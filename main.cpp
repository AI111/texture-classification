    ...
    for (int i = 0; i <imeges.size() ; i++) {
        for (int j = 0; j <imeges[i].size() ; j++) {
           // imshow("img "+to_string(i)+" img # "+to_string(j),imeges[i][j]);
            ...
            //cout<<"Max ="<<max<<endl;
            max*=0.51;//MaxLmaxoc(dst,&min,&max);
            //cout<<"Max ="<<max<<endl;
            threshold(dst,dst,max,max,cv::ThresholdTypes::THRESH_TOZERO);
            //imshow("Threshold spectrum",dst);
            medianBlur(dst,dst,5);
            //threshold(dst,dst,0.49*max,1,cv::ThresholdTypes::THRESH_BINARY);
...
//            Mat dst;//(512,512,CV_32F);
//            Mat entrop = calcEntrop1(imeges[1][0],64);
//
//            cout<<entrop<<endl<<endl;
//            normalize(entrop, entrop, 0, 255, NORM_MINMAX);
//            cout<<entrop<<endl<<endl;
//
//            imshow("entrop 2" ,entrop);
//            //cout<<entrop;
//            lbp::dftShift(entrop,dst);
//            //imshow("spectrum",dst);
//            //cout<<dst;
...
//            minMaxLoc(dst,&min,&max);
//            //cout<<"Max ="<<max<<endl;
//            max*=0.5;//MaxLoc(dst,&min,&max);
//            //cout<<"Max ="<<max<<endl;
//            threshold(dst,dst,max,max,cv::ThresholdTypes::THRESH_TOZERO);
//            //imshow("Threshold spectrum",dst);
//            cout<<dst;
//            medianBlur(dst,dst,5);
//            imshow("Threshold spectrum median class "+to_string(0)+" img # "+to_string(0),dst*255);
